...
headers = {
        ...
        "x-apikey": None  # Will be set by user input
        }
...
# Utility to sanitize drag-and-drop file paths from Windows terminal
def sanitize_path(path):
    if not path:
         return ''
# Remove leading & and quotes, and trailing quotes/spaces
    path = path.strip().lstrip("& ").strip("'\"").rstrip("'\"").strip().rstrip('\\')
    return path
...
def APICheck():
    ...
    if not headers["x-apikey"]:
            api_key = input("Enter your VirusTotal API Key (leave blank to skip VirusTotal features): ").strip()
            if api_key:
                headers["x-apikey"] = api_key
            else:
                print("You will not be able to use VirusTotal API features without an API key.")
...
#SHA256 Option
def antivirus_SHA():
    SHA = input("Input the SHA that you want to scan, press '1' to go back:").strip().rstrip('\\')
    if SHA == '1':
        start()
    else:
        file_path = None
